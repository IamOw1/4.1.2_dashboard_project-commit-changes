"""
Субагент для контроля, советов и анализа с поддержкой локального LLM
"""
import asyncio
import json
import logging
...
from .llm_client import get_llm_client, generate_with_fallback
...
class SubAgentConfig:
    ...
    use_local_llm: bool = True  # Prefer local LLM
    local_llm_endpoint: str = "http://localhost:11434"
    local_llm_model: str = "deepseek-coder"
...
class SubAgent:
    ...
    def __init__(self, config: Dict[str, Any], main_agent=None):
        ...
        if self.config.use_local_llm:
            try:
                ...
                llm_config = LLMConfig(
                    ...
                    endpoint_url=self.config.local_llm_endpoint
                )
        ...
        logger.info("Субагент инициализирован")
    ...
    async def ask(self, question: str) -> str:
        ...
        try:
            ...
            context = {
                ...
                'conversation_history': self.conversation_history[:-1]  # Exclude current question
            }
            
            # Try local LLM first
            if self.llm_client:
                try:
                    llm_response = await self.llm_client.generate(question, context)
                    if not llm_response.error and llm_response.text:
                        answer = llm_response.text
                        logger.info("Response generated using local LLM")
                        
                        # Добавление ответа в историю
                        self.conversation_history.append({"role": "assistant", "content": answer})
                        return answer
                    else:
                        logger.warning(f"Local LLM failed: {llm_response.error}")
                except Exception as e:
            ...
            # No LLM available
            return "No LLM service available (neither local nor cloud API)"
