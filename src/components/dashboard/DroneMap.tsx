...
import type { FleetDrone, Mission, DetectedObject, GeoZone } from "@/lib/mock-data";
import { detectedObjectLabel, missionTypeLabel } from "@/lib/mock-data";

type LeafletModule = typeof import("leaflet");
type LeafletMapInstance = import("leaflet").Map;
type LeafletTileLayerInstance = import("leaflet").TileLayer;
type LeafletLayerGroupInstance = import("leaflet").LayerGroup;

export type MapProvider = "osm" | "satellite" | "topo" | "dark";
...
export const mapProviderList = Object.entries(tileProviders).map(([id, v]) => ({
  id: id as MapProvider,
  name: v.name,
}));

// SVG-маркер дрона с поворотом по heading
function droneIcon(L: LeafletModule, name: string, heading: number, color = "#06b6d4", isActive = false) {
...
function objectIcon(L: LeafletModule, type: string) {
...
function waypointIcon(L: LeafletModule, index: number, isLast = false) {
...
interface DroneMapProps {
  ...
  /** Положение наземного шлюза (оператора). Если задано вместе с activeDroneId — рисуем mesh-линию связи. */
  gatewayPosition?: [number, number];
  /** Показывать линию связи Gateway → активный дрон (с авто-маршрутом через сильнейший пир при слабом сигнале). */
  showMeshLink?: boolean;
  /** Показывать топологию mesh-сети между всеми активными дронами флота. */
  showFleetMesh?: boolean;
}
...
export function DroneMap({
  ...
  const layerGroupRef = useRef<LeafletLayerGroupInstance | null>(null);
  // Гистерезис для режима маршрута: вход в multi-hop при signal<60, выход при signal≥70.
  // Запоминаем предыдущий режим и id транзитного дрона, чтобы трасса не дёргалась на границе.
  const routeStateRef = useRef<{ mode: "direct" | "hop"; hopId: string | null }>({
    mode: "direct",
    hopId: null,
  });
  // Текущий режим маршрута для отображения бейджа над картой (стейт-лайт через ref + force update).
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    // Inject leaflet CSS once on the client to avoid SSR resolution of the package.
  ...
  useEffect(() => {
    ...
    layersData.geoZones.forEach((z) => {
      ...
      const polygon = L.polygon(z.polygon, {
        ...
        fillColor: c.fill,
        fillOpacity: 0.08,
        dashArray: z.type === "restricted" ? "6 6" : undefined,
      });
      polygon.bindTooltip(z.name, { sticky: true, className: "leaflet-tooltip-coba" });
      grp.addLayer(polygon);
    });

    // mesh link: gateway → (опц. транзитный пир) → активный дрон
    // Гистерезис: вход в multi-hop при signal<60, выход обратно в direct при signal≥70.
    // Между порогами сохраняем предыдущий режим, чтобы линия не моргала на границе.
    if (
      layersData.showMeshLink &&
      layersData.gatewayPosition &&
      layersData.activeDroneId
    ) {
      ...
      if (active && active.status !== "offline" && active.status !== "maintenance") {
        ...
        // Решение о режиме с гистерезисом
        let mode: "direct" | "hop" = prev.mode;
        if (active.signal < 60) mode = "hop";
        else if (active.signal >= 70) mode = "direct";
        // в зоне 60..69 — оставляем предыдущий

        // Поиск транзитного пира (если нужен hop). Стараемся сохранить предыдущий hop,
        // если он всё ещё доступен и сильный — это снижает дрожание трассы.
        const peerCandidates = layersData.drones.filter(
          (d) =>
            d.id !== active.id &&
            (d.status === "mission" || d.status === "online") &&
            d.signal >= 70,
        );
        let hop: FleetDrone | null = null;
        if (mode === "hop" && peerCandidates.length) {
          const sticky = prev.hopId
            ? peerCandidates.find((p) => p.id === prev.hopId)
            : undefined;
          hop =
            sticky ??
            peerCandidates
        ...
        if (mode === "hop" && !hop) {
          // нет подходящего пира — деградируем в direct
          mode = "direct";
        }

        // Обновляем состояние маршрута и бейджа только при реальной смене —
        // setRouteBadge безопасен, т.к. изменение объекта запускает ререндер только при diff.
        ...
        setRouteBadge((curr) => {
          ...
          if (
            curr &&
            curr.mode === next.mode &&
            curr.hopName === next.hopName &&
            curr.signal === next.signal
          ) {
            return curr;
          }
          return next;
        ...
        // Класс fade-in добавляется при смене режима/транзита, чтобы новая трасса проявлялась плавно.
        const fadeClass =
          prev.mode !== mode || prev.hopId !== newHopId ? " mesh-link-fade" : "";
        ...
        // маркер шлюза (наземный оператор)
        ...
        // активный дрон недоступен — гасим бейдж
        setRouteBadge(null);
      }
    } else {
      // mesh-линия не показывается — гасим бейдж
      setRouteBadge((curr) => (curr === null ? curr : null));
    }

    // fleet-wide mesh topology: тонкие линии между активными дронами в радиусе связи
    if (layersData.showFleetMesh) {
      const peers = layersData.drones.filter(
        (d) => d.status === "mission" || d.status === "online",
      );
      const MAX_KM = 3.0; // радиус устойчивой связи между узлами
      for (let i = 0; i < peers.length; i++) {
        for (let j = i + 1; j < peers.length; j++) {
          ...
          const strength = Math.max(0, 1 - km / MAX_KM); // 0..1
          ...
          grp.addLayer(
            L.polyline([a.position, b.position], {
              ...
              dashArray: strength > 0.5 ? undefined : "4 4",
            }),
          );
        }
      }
    ...
    layersData.missions.forEach((m) => {
      ...
      const path = m.waypoints.map((w) => [w.lat, w.lng] as [number, number]);
      if (path.length > 1) {
        const polyline = L.polyline(path, {
          color: m.status === "running" ? "#06b6d4" : isActive ? "#fbbf24" : "#64748b",
          weight: isActive ? 3 : 2,
          opacity: isActive ? 0.95 : 0.5,
          dashArray: m.status === "completed" ? undefined : "8 6",
        });
    ...
    layersData.drones.forEach((d) => {
      ...
      const missionInfo = d.mission
        ? layersData.missions.find((m) => m.id === d.mission)
        : null;
      marker.bindPopup(
    ...
    layersData.detections.forEach((o) => {
      ...
      marker.bindPopup(
        `<div style="font:700 12px Inter,sans-serif;color:${objectColors[o.type] ?? "#94a3b8"};margin-bottom:6px;">
           ${detectedObjectLabel[o.type as keyof typeof detectedObjectLabel] ?? o.type}
         </div>
         <div style="font:11px 'JetBrains Mono',monospace;color:#94a3b8;line-height:1.6;">
  ...
  return (
    <div className="relative h-full w-full" style={{ height }}>
      ...
      {/* Бейдж режима маршрута связи (direct ↔ multi-hop) */}
      {showMeshLink && routeBadge && (
        <div
          key={`${routeBadge.mode}-${routeBadge.hopName ?? "none"}`}
        ...
        >
          ...
          <span className="text-foreground">
            {routeBadge.mode === "direct"
              ? "DIRECT"
              : `MULTI-HOP · ${routeBadge.hopName ?? ""}`}
          </span>
          <span className="text-muted-foreground">SIG {routeBadge.signal}%</span>
        </div>
      )}
      {showProviderSwitcher && (
        <div className="absolute right-3 top-3 z-[400] flex flex-col gap-1 rounded-md border border-border bg-background/85 p-1 backdrop-blur">
          {mapProviderList.map((p) => (
            <button
              key={p.id}
